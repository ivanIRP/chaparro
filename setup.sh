...
echo "" > /etc/frr/frr.conf; systemctl restart frr
sleep 1
...
case $OPC in
    0) # R0
        ...
        $IP link set tunR1 up; $IP addr add 2000::101/126 dev tunR1
        
        $VTY -c "conf t" -c "ipv6 route 2000::/110 2000::102" -c "exit" -c "write"
        ;;
    1) # R1
        ...
        $IP link set tunR0 up; $IP addr add 2000::102/126 dev tunR0
        
        $IP link add tunR3 type ip6gre local 2000::19 remote 2000::22
        $IP link set tunR3 up; $IP addr add 10.0.0.1/30 dev tunR3
        
        $VTY -c "conf t" \
             -c "ipv6 route 2000::20/125 2000::1a" \
             -c "ipv6 route 2000::/125 2000::101" \
             -c "ip route 9.0.0.0/24 10.0.0.2" \
             -c "ip route 11.0.0.0/24 10.0.0.2" \
             -c "ip route 200.0.0.4/30 10.0.0.2" \
             -c "ip route 200.0.0.8/30 10.0.0.2" -c "exit" -c "write"
        ;;
    2) # R2
    ...
    3) # R3
        ...
        $IP link add tunR1 type ip6gre local 2000::22 remote 2000::19
        $IP link set tunR1 up; $IP addr add 10.0.0.2/30 dev tunR1
        
        $IP tunnel add tunR4 mode sit remote 200.0.0.6 local 200.0.0.5
        $IP link set tunR4 up; $IP addr add 2000::201/126 dev tunR4
        
        $VTY -c "conf t" \
             -c "ipv6 route 2000::18/125 2000::21" \
             -c "ipv6 route 2000::/125 2000::21" \
             -c "ipv6 route 2000::8/125 2000::21" \
             -c "ipv6 route 2000::10/125 2000::202" \
             -c "ip route 8.0.0.0/24 10.0.0.1" \
             -c "ip route 200.0.0.0/30 10.0.0.1" \
             -c "ip route 11.0.0.0/24 200.0.0.6" -c "exit" -c "write"
        ;;
    4) # R4
        ...
        $IP link set tunR3 up; $IP addr add 2000::202/126 dev tunR3
        
        $VTY -c "conf t" \
             -c "ip route 8.0.0.0/24 200.0.0.5" \
             -c "ip route 9.0.0.0/24 200.0.0.5" \
             -c "ip route 11.0.0.0/24 200.0.0.10" \
             -c "ipv6 route 2000::/110 2000::201" -c "exit" -c "write"
        ;;
    5) # R5
...
echo "[!] R$OPC configurado con precisión. Prueba los pings."